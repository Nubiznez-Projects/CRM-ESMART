...
export const AddCompanyProfile = ({
  ...
  const validationSchema = Yup.object().shape({
    companyProfile: Yup.mixed().required("Profile image is required"),
    ...
    typeOfBusiness: Yup.string()
      .oneOf(
        businessOptions
          .map((option) => option.value)
          .filter((value) => value !== ""),
        "Please select a valid Type of Industry"
      )
  ...
  const handleSubmit = async (values) => {
    try {
      const response = await SubmitClientDetails(
        values,
        clientId,
        setClientID,
        clientData?.company
      ...
      >
        {({ handleSubmit, values, errors, touched, setFieldValue }) => (
          <Form onSubmit={handleSubmit}>
            <div className="flex justify-center items-start mt-[1vw] relative">
