...
import { SubmitClientExcel } from "../../API/UserManagement/Client/ClientDetails";
...
export default function UserManagement() {
  ...
  const handleExcelSubmit = async (file) => {
    ...
    try {
      let response;
      if(tabName === "Client" ){
      response = await SubmitClientExcel(file);
      dispatch(fetchClient());
      } else{
        response = await SubmitEmployeeExcel(file);
    ...
    <>
        ...
        <div className="flex">
          {tabName === "Client" ? (
            <p className="text-[0.8vw]">
              ...
              {Math.min(itemOffset + itemsPerPage, client?.length)} of{" "}
              {client?.length} items
            </p>
          ) : tabName === "Employee" ? (
            <p className="text-[0.8vw]">
      ...
      >
        {tabName === "Employee" ? (
          <DeleteList
            setDeleteModalIsOpen={setDeleteModalIsOpen}
            title={`Want to delete this ${empName}`}
