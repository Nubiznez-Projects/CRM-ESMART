...
import { getSupplierByID } from "../../../API/MasterModule/Supplier";
...
export default function SupplierList({
 ...
 console.log(supplierId, "supplier_Data");
  ...
  const columns = [
    {
      title: <span className="text-[1vw] font-semibold">Supplier ID</span>,
      align: "center",
      width: "15vw",
      render: (row) => {
        return (
          <div className="text-[0.85vw] font-medium ">{row?.SupplierID}</div>
        );
      },
    },
    {
      title: <span className="text-[1vw] font-semibold">Supplier Name</span>,
      align: "center",
      width: "15vw",
      sorter: (a, b) => a?.SupplierName?.localeCompare(b?.SupplierName),
      render: (row) => {
        return <div className="text-[0.85vw] ">{row?.SupplierName}</div>;
      },
    },
    {
      title: <span className="text-[1vw] font-semibold">Supplier Type</span>,
      align: "center",
      width: "15vw",
      sorter: (a, b) => a?.SupplierType?.localeCompare(b?.SupplierType),
      render: (row) => {
        return <div className="text-[0.85vw] ">{row?.SupplierType}</div>;
      },
    },
    {
      ...
      render: (row) => {
        return <div className="text-[0.85vw] ">{row?.ContactPersonName}</div>;
      },
    },
    {
          ...
          <div className="px-[1vw] flex gap-[1.1vw]">
            ...
            <div
              ...
              onClick={() => {
                setSupplierId(row.SupplierID);
                setOpenAddModal(true);
                console.log(row.SupplierID, "supplier_id");
              }}
            >
              <VscEdit size={"1.2vw"} />
            </div>
  ...
  const pageCount = supplier && itemsPerPage ? Math.ceil(supplier?.length / itemsPerPage) : 0;
  console.log(pageCount, itemsPerPage, pageNumber, itemOffset, "page");
    ...
    <>
      ...
      <div className="fixed bottom-[5vh] w-screen flex justify-between items-center">
        <div className="flex">
          <p className="text-[0.8vw]">
            ...
            {Math.min(itemOffset + itemsPerPage, supplier?.length)} of{" "}
            {supplier?.length} items
          </p>
        </div>
      ...
      >
        <DeleteList
          ...
          title={`Want to delete this ${supplierData?.SupplierName}`}
          api={`${apiUrl}/suppliers/${supplierId}`}
          module={"supplier"}
          // filter={filter}
