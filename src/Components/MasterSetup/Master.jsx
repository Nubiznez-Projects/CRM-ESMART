...
import SupplierList from "./Supplier/SuplierList";
...
} from "../../Redux/Slice/MasterModule/Customers/CustomerThunks";
import { fetchFilteredSupplier } from "../../Redux/Slice/MasterModule/Suppliers/SupplierThunks";
import ExportButton from "../Common/Export/ExportButton";

const { RangePicker } = DatePicker;

export default function Master() {
  const [tabName, setTabName] = useState("Supplier");
  ...
  const handleSearchChange = (e) => {
    ...
    if (tabName === "Customer") {
      setSearchTerm(value);
      dispatch(fetchSearchedCustomers(value)); // dispatch search
    }
  };
...
const handleStatusChange = (value) => {
  ...
  if (value === 2) selectedStatus = "active";
  else if (value === 3) selectedStatus = "inactive";
...
// Dispatcher for filtering
const applyFilters = (fromDate, toDate, status) => {
  const filters = { fromDate, toDate, status };

  switch (tabName) {
    case "Supplier":
      dispatch(fetchFilteredSupplier(filters));
      break;
    case "Customer":
    default:
      dispatch(fetchFilteredCustomers(filters));
      break;
  }
};
  ...
  if (key === "Supplier") {
    dispatch(fetchFilteredSupplier({ fromDate: null, toDate: null, status: "" }));
  } else {
    dispatch(fetchFilteredCustomers({ fromDate: null, toDate: null, status: "" }));
  }
  };
          ...
          <div className="flex flex-col gap-y-[0.5vw] mt-[1vw]">
            <div className="flex gap-[1.5vw] justify-end ml-auto">
              <div>
                ...
                >
                  ...
                  <span className="font-bold text-white text-[0.9vw]">
                    {tabName === "Supplier" ? "Add Supplier" : "Add Customer"}
                  </span>
                </button>
              </div>
              <div>
              ...
              <div>
                ...
                />
                <ExportButton
                    dataArray={ tabName == "Supplier" ? supplier || [] : list || [] }
                  />
                {/* <button
                  className="bg-[#ffffff] border border-[#ebebeb] shadow-md flex px-[0.75vw] h-[2.25vw] w-[9vw] 
                 justify-center gap-[0.5vw] items-center rounded-[0.6vw] 
              ...
              <div className="">
                ...
                >
                  <Select
                    ...
                    value={
                      status === "active" ? 2 : status === "inactive" ? 3 : null
                    }
                  />
