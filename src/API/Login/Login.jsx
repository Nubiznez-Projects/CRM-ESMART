...
export const login = async (userType, values) => {
  ...
  const payload = { email: values?.email_id, password: values?.password };

  try {
    const response = await axios.post(`${BASE_URL}/prologin/login`, payload); // send payload directly
